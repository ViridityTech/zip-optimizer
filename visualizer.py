...
def main():
    ...
    **Workflow:**
    ...
    2. Upload the **ZIP Code Boundaries GeoJSON** file.
    3. The app loads ZIP code boundaries and merges them with the uploaded data.
    4. For each clinic, a Folium map is generated showing the assigned ZIP codes.
       Tooltips display all individual metric values.
    5. Download a ZIP file containing one HTML map per clinic.
    """)
    ...
    with st.sidebar:
        ...
        uploaded_geojson = st.file_uploader(
            "Upload ZIP Code Boundaries GeoJSON",
            type=["json", "geojson"],
            help="GeoJSON file containing ZIP code boundary polygons (e.g., usa_zip_codes_geo_100m.json)"
        )
    ...
    if not uploaded_csv or not uploaded_geojson:
        st.warning("Please upload both required files to continue.")
        st.info("""
        **Required Files:**
        
        1. **Optimized Assignments CSV** - Generated by the Optimizer tool
        2. **ZIP Code Boundaries GeoJSON** - Contains ZIP code boundary polygons
           - Must have a column containing ZIP codes (e.g., ZCTA5CE10, GEOID10, etc.)
           - Must contain geometry data for each ZIP code
        """)
        if not run_button:
            st.info("Upload files and click **Generate Maps**.")
        return
    ...
    try:
        zip_geo = gpd.read_file(uploaded_geojson)
        st.success("✓ ZIP code boundaries GeoJSON loaded successfully.")
    except Exception as e:
